...
; Programa Imagojo
; AutoIt 3.3.61
; Creado :12/11/10   Modificado: 10/07/11 
...
Global $ALIAS2="", $AE2="", $IP2="", $PUERTO2="", $ALIAS1="", $AE1="", $IP1="", $PUERTO1="", $SNIFFER, $OPSAL=1
...
Global $Ruta_video, $form23, $form24, $form25, $BorrarEnv, $Dir_trab, $debug=1
...
$ALIAS2=IniRead("Imagojo.ini", "Servidores", "Alias2", 'xxSYNEDRA')
$AE2=IniRead("Imagojo.ini", "Servidores", "AE2", 'xxSYNEDRA_VIEW')
$IP2=IniRead("Imagojo.ini", "Servidores", "IP2", 'xxlocalhost')
$PUERTO2=IniRead("Imagojo.ini", "Servidores", "PUERTO2", 'xx11317')
$SNIFFER=IniRead("Imagojo.ini", "Servidores", "SNIFFER", 'xx11317')


$Servidor = IniRead("Imagojo.ini", "Servidores", "Servidor", 'xxCharrua')
$IpPuertoServidor = IniRead("Imagojo.ini", "Servidores", "IpPuertoServidor", 'xx192.168.1.101:1112')
$Bucle=IniRead("Imagojo.ini", "EnvioDicom", "Reintentos", '4')
$BorrarEnv=IniRead("Imagojo.ini", "EnvioDicom", "BorrarEnviados", '0')
...
;Inico con el ingreso del Usuario
$mensaje = ""
While $hab
$usuario = InputBox("Programa Imagojo", $mensaje & "Ingrese Usuario") 
if @error = 1 then Exit
if $usuario = "Alvaro" then
	exitloop
ElseIf $usuario = "Rodrigo" then
	exitloop
Else
	$mensaje ="Atención: No registrado"& @lf
EndIf
WEnd
...
if @error = 1 then Exit
if $contrasenia = "valida" then ExitLoop
WEnd	
;#cs
;Se hace un EchoDicom al empezar
	FileDelete(@TempDir & "\Imagojo.log")
	RunWait( @comspec & " /c dcmecho "&$AE1&"@"&$IpPuertoServidor & " > " & @TempDir & "\Imagojo.log",$dir_trab,@SW_HIDE)
	$file = FileOpen(@TempDir & "\Imagojo.log", 0)
	If $file = -1 Then
...
;$Pic1 = GUICtrlCreatePic("H:\Imagojo\Software bajado\Programacion\Logo\bandacolor.gif", 0, 0, 801, 40)
If $EstadoServ = "Activo" Then $Pic1 = GUICtrlCreatePic($Ruta_Imagojo&"\bandacolorV.gif", 0, 0, 801, 40)
If $EstadoServ = "Inactivo" Then $Pic1 = GUICtrlCreatePic($Ruta_Imagojo&"\bandacolor.gif", 0, 0, 801, 40)
...
$MenuItem12 = GUICtrlCreateMenuItem("Explorar Carpeta del Visualizador ", $MenuItem9)
...
Case $MenuItem12; Explorar Carpeta Visor-Editor
	$path=@TempDir & "\synedraViewPersonal"
...
$Form2 = GUICreate("Imagojo: Paciente", 801, 451, -1)
GUISetBkColor(0xF9FCFF)
...
If not _DateIsValid( $sDate ) or StringLen ($DiaN_pa)>2 or StringLen ($MesN_pa)>2 Then
  MsgBox( 4096, "ATENCIÓN", "La fecha de nacimiento ingresada no es válida")
  $bien=0
EndIf

if $ced_pa = "" or $Nom_pa = "" or $Ape_pa = "" or $DiaN_pa = "" or $MesN_pa = "" Or $AniN_pa = "" or $sex_paciente = "" Or $bien=0 then 
	$noingresa=1
	MsgBox(4096, "ATENCIÓN", "Debe ingresar los datos marcados con *",10)
Else 
	$noingresa=0
EndIf
if $noingresa=0 then	
...
;$var = FileOpenDialog($message, "C:\ZImagojo\", "Imagenes (*.jpg)", 1 + 4 )
...
$Form5 = GUICreate("Imagen seleccionada para crear Dicom para paciente: "&$nom_paciente,801, 444, -1, -1, $WS_SIZEBOX + $WS_SYSMENU)
...
GUIDelete ($form5)
Call ("Form"&$op)
$escala=1;al salir siempre vuelve a la escala chica
...
FileClose($file)
;Crear imagen Dicom
;Antes crear un número al azar de cuatro cifras para que los mombres no se repitan
$Azar=0
While $Azar<1000
...
;Se copia el archivo seleccionado con otro nombre mas corto, sino el jpg2dcm da problemas - Probar en un temporal f f f f f f f f f f f f f f f f
...
if $result >0 then
	...
	;MsgBox(0, "Resultado:", "El archivo "& $Nom_Dcm&$Azar &" fue creado con éxito"& @CRLF & "en la carpeta "&$Ruta_Dcm,"")
	GUIDelete ($form5)
Else
...
;OK-->  PARA MANDAR UNA IMAGEN.DCM AL CHARRUAPACS
RunWait( @comspec & " /c dcmsnd -stgcmt -L "&$AE2&":"&$PUERTO2&" "&$AE1&"@"&$IpPuertoServidor&" "&$Arch_nuevo_dcm & " > " & @TempDir & "\Temp"&$Azar&".log",$dir_trab,@SW_HIDE)
;Lee el log
...
WEnd
;si llegó al final
MsgBox(0, "ReSultado:", "No hay conexión con el servidor el archivo pasará a la carpeta Atrasados para enviarse mas tarde","")
...
SplashOff()
$Form6 = GUICreate("Imagojo Configuración", 801, 451, -1, -1)
...
;7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7 CREAR Y ENVIAR PDF 7 7 7 7 7 7 7 7 7 7 7 7 7 7 7
Func Form7 ();Captura
MsgBox(0, "ReSultado:", "Este equipo no tiene función de Adquisición de Imagenes","")
;RunWait( @comspec & " /c pdf2dcm -c C:\Zimagojo\homer.cfg C:\Zimagojo\homer.jpg C:\Zimagojo\Pepito.dcm " & " > " & @TempDir & "\conversión.log","",@SW_HIDE)
Call ("Form1")
...
$Form8 = GUICreate("Imagojo Creación de Archivo Dicom a partir de:", 801, 424, -1, -1)
GUISetBkColor(0xF9FCFF)
...
$Label1 = GUICtrlCreateLabel("Paciente: ", 120, 200, 87, 28)
GUICtrlSetFont(-1, 14, 400, 0, "MS Sans Serif")
$Label2 = GUICtrlCreateLabel($Nom_pa &" "&$Ape_pa, 203, 201, 147, 28)
GUICtrlSetFont(-1, 14, 400, 0, "MS Sans Serif")
$Label4 = GUICtrlCreateLabel("Estación : "&$Estacion, 118, 144, 193, 28)
GUICtrlSetFont(-1, 14, 400, 0, "MS Sans Serif")
#EndRegion ### END Koda GUI section ###
...
$Pic1 = GUICtrlCreatePic($Ruta_Imagojo&"\PdfImage.jpg", 136, 117, 249, 281)
...
Call ("Form8")
Case $Button1; Crear
	$desc_estud=GUICtrlRead($Input1)
	$Coment=GUICtrlRead($Input2)
	$op=22;Va a Form22 crear dicom con Pdf
	ExitLoop
Case $Button2; Cancelar
	$op=8;Vuelve al menú Crear Dicom
	ExitLoop
EndSwitch
WEnd
GUIDelete ($form21)
...
;OK-->  PARA MANDAR UNA IMAGEN.DCM AL CHARRUAPACS
If $debug=1 then
$resp=MsgBox(4, "Debug", "¿Quiere que se haga en una ventana de comandos?")
	if $resp=6 Then
	Run ( 'cmd.EXE' )
	Sleep ( 1500 )
	Send ("cd C:\Imagojo\dcm4\bin{ENTER}")
	Sleep ( 500 )
	Send ("dcmsnd -stgcmt -L "&$AE2&":"&$PUERTO2&" "&$AE1&"@"&$IpPuertoServidor&" "&$Arch_nuevo_dcm &"{ENTER}")
	Sleep ( 30000 )
	MsgBox(0, "Enviar Dicom", "Y como anduvo la cosa?")
	EndIf
EndIf
;RunWait( @comspec & " /c dcmsnd -stgcmt -L DCMSND:11113 STORESCP@localhost:844 C:\Zimagojo\dcm4\bin\REPORT.dcm" & " > " & @TempDir & "\dirlist.txt","",@SW_HIDE)
RunWait( @comspec & " /c dcmsnd -stgcmt -L "&$AE2&":"&$PUERTO2&" "&$AE1&"@"&$IpPuertoServidor&" "&$Arch_nuevo_dcm & " > " & @TempDir & "\Temp"&$Azar&".log",$Dir_trab,@SW_HIDE)
;Lee el lg
$file = FileOpen(@TempDir & "\Temp"&$Azar&".log", 0)
; Check if file opened for reading OK
...
WEnd
;si llegó al final
MsgBox(0, "ReSultado:", "No se envió, el archivo pasará a la carpeta NoEnviados para enviarse mas tarde","")
;FileWriteLine
;FileDelete (@TempDir & "\Temp"&$Azar&".log")
GUIDelete ($form8)
Call ("Form8")
EndFunc;fin de Func21
...
;    MsgBox(4096,"","Archivo elegido =" & $Ruta_video)
 ;   MsgBox(4096,"","$arch_pdf =" & $arch_video)
	EndIf
;$nom_paciente = $Nom_pa &" " & $Ape_pa
...
$Pic1 = GUICtrlCreatePic($Ruta_Imagojo&"\MpgImage.jpg", 136, 117, 249, 281)
$Button1 = GUICtrlCreateButton("Crear", 432, 56, 75, 25, 0)
$Button2 = GUICtrlCreateButton("Cancelar", 16, 56, 75, 25, 0)
$Label1 = GUICtrlCreateLabel("Descripción del estudio", 8, 8, 114, 17)
...
;ruta y archivo dicom a crear:
;ej.: C:\Documents and Settings\Alvaro\Configuración local\Temp\Imagojo2345.dcm - Sacar nombre al azar y ponerle año-mes-dia-hora-min-seg f f f f f f f f f f f f f f f f
$Nom_Dcm=@YEAR&@MON&@MDAY&@HOUR&@MIN&@SEC
$Arch_nuevo_dcm = $Ruta_Dcm &"\"& $Nom_Dcm &".dcm"
;$Arch_nuevo_dcm = $Ruta_Dcm &"\"& $Nom_Dcm&$Azar &".dcm"
$consola_crea_log = @TempDir & "\conversión.log";log de la consola al crear el archivo
...
;invitar a enviar Dicom con la opción a elegir a que servidor quiere mandarla
; o invitar a abrir el archivo con algún Visor Dicom
$respuesta=MsgBox(4, "Archivo crado exitosamente", "Desea enviar al servidor "&$servidor &" el archivo dicom creado ahora?")
if $respuesta= "7" Then
	GUIDelete ($form8)
	Call ("Form8")
	EndIf
...
;MsgBox(0, "Intento "&$n, "retardo de "&$retardo/1000,5)
;OK-->  PARA MANDAR UNA IMAGEN.DCM AL CHARRUAPACS
; ENVIAR ARCHIVO MPG VIDEO           M P G     VIDEO           M P G     VIDEO
;RunWait( @comspec & " /c dcmsnd -stgcmt -L DCMSND:11113 STORESCP@localhost:844 C:\Zimagojo\dcm4\bin\REPORT.dcm" & " > " & @TempDir & "\dirlist.txt","",@SW_HIDE)
RunWait( @comspec & " /c dcmsnd -stgcmt -L "&$AE2&":"&$PUERTO2&" "&$AE1&"@"&$IpPuertoServidor&" "&$Arch_nuevo_dcm & " > " & @TempDir & "\TempMPG"&$Azar&".log",$Dir_trab,@SW_HIDE)
;Leer el log
$file = FileOpen(@TempDir & "\TempMPG"&$Azar&".log", 0)
; Check if file opened for reading OK
If $file = -1 Then
    MsgBox(0, "Error 206", "Chequear instalación")
    Exit
EndIf
...
MsgBox(0, "Resultado:", "El archivo fue enviado correctamente",10)
Sleep (100)
;FileDelete (@TempDir & "\TempMPG"&$Azar&".log")
If $BorrarEnv=1 Then FileDelete ($file, $Arch_nuevo_dcm)
GUIDelete ($form8)
Call ("Form8")
EndIf
;Si no fue enviado incrementa el contador
...
WEnd
;si llegó al final
MsgBox(0, "ReSultado:", "NO PUDO SER ENVIADO, EL archivo pasará a la carpeta Atrasados para enviarse mas tarde","")
$file = FileOpen("NoEnviados.log", 1)
...
$Image1 = GUICtrlCreatePic("C:\Imagojo\Acercade2-300.gif", 16, 24, 169, 153)
...
$Pic1 = GUICtrlCreatePic("C:\Imagojo\Imagojo5.gif", 262, 26, 33, 33)
