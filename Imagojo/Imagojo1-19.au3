...
Case $Consultar;Consulta a la base de datos para ver si ya existe el paciente, y en caso afirmativo trae los datos
	...
	$ced_pa = GUICtrlRead($ced)
	Dim $UserName = "root"
	Dim $MySql_number = "4624"
	Dim $Database = "pacsdb"
	Dim $MySQLServerName = "localhost"
	Dim $TableName = "patient"


	$SQLInstance = _MySQLConnect ($UserName, $MySql_number, $Database, $MySQLServerName)

	$pat_id_Pacsdb= ""
	$pat_name_Pacsdb= ""
	$pat_birthdate_Pacsdb= ""
	$pat_sex_Pacsdb= ""
	...
	With $TableContents
		While Not .EOF
			$pat_id_Pacsdb &= .Fields ("pat_id").value
			$pat_name_Pacsdb &= .Fields ("pat_name").value
			$pat_birthdate_Pacsdb &= .Fields ("pat_birthdate").value
			$pat_sex_Pacsdb &= .Fields ("pat_sex").value
			$pat_id_issuer_Pacsdb &= .Fields ("pat_id_issuer").value
			IF $pat_id_Pacsdb= $ced_pa THEN ExitLoop
				$pat_id_Pacsdb= ""
				$pat_name_Pacsdb= ""
				$pat_birthdate_Pacsdb= ""
				$pat_sex_Pacsdb= ""
				$pat_id_issuer_Pacsdb= ""
			;$NameList &= .Fields ("pat_id").value & " " & .Fields ("pat_name").value & " " & .Fields ("pat_birthdate").value  & @CRLF
			.MoveNext
		WEnd
	EndWith
	_MySQLEnd ($SQLInstance)
	If $pat_id_Pacsdb="" Then
		MsgBox(0, "Búsqueda finalizada", "No se encontraron datos para el IdPaciente",10)
		ExitLoop
		EndIf
