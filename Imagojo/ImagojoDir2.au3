...
#AutoIt3Wrapper_Icon=C:\ZProgram\iconos\IMAGOJODIR1.ico
#EndRegion
#include <Date.au3>
#include <IE.au3>
...
;LECTURA DEL ARCHIVO DE CONFIGURACIÓN IMAGOJO2.ini
$ALIAS1=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Comunicaciones", "Alias1", 'xxCharrua')
$AE1=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Comunicaciones", "AE1", 'xxCHARRUAPACS')
$IP1=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Comunicaciones", "IP1", 'xxlocalhost')
$PUERTO1=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Comunicaciones", "PUERTO1", 'xx11112')
$AE2=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Comunicaciones", "AE2", 'xxESTACIONX')
$PUERTO2=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Comunicaciones", "PUERTO2", 'xx11113')
...
$Bucle=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "EnvioDICOM", "Reintentos", '4')
$Ocultar=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "EnvioDICOM", "Ocultar", '0')

;directorio de trabajo de Dcm4che
$Dir_trab=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Java", "Dcm4che_bin", 'C:\IMAGOJO\dcm4\bin\')
$Ruta_IMAGOJO=IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Directorios", "Ruta_IMAGOJO", 'C:\IMAGOJO')
;Directorio de archivos DICOM a enviar
$Ruta_Dcm= IniRead("C:\IMAGOJO\IMAGOJO2.ini", "Directorios", "Ruta_Dcm", 'C:\IMAGOJO\Dcm')
;YYYYMMDDHHMMSS
;IniWrite($Ruta_IMAGOJO&"\IMAGOJO2.ini", "IMAGOJODIRFref", "FRef", '20110720000000')
;IniWrite($Ruta_IMAGOJO&"\IMAGOJO2.ini", "Directorios", "Ruta_DcmF", 'C:\IMAGOJO\NoEnviados')

$FRef=IniRead($Ruta_IMAGOJO&"\IMAGOJO2.ini", "Fechas", "FRef", '20110720000000')
;$FRef=IniRead($Ruta_IMAGOJO&"\IMAGOJO2.ini", "IMAGOJODIRFref", "FRef", '20110720000000')
...
;    MsgBox(4096, "$arch:", $arch)
	...
	if $t>$FRef Then
		$K=$K+1
		IF $K=50 THEN SplashTextOn ("IMAGOJODIR", "Buscando nuevos archivos DICOM para enviar", 250,100)
EndIf
WEnd
SplashOff()
...
FileChangeDir($Dir_trab)
	FileDelete(@TempDir & "\ECO_2.log")
	GUICtrlSetData ($progress1,30)
	RunWait( @comspec & " /c dcmecho "&$AE1&"@"&$IpPuertoServidor & " > " & @TempDir & "\ECO_2.log",$Dir_trab,@SW_HIDE)
	$file = FileOpen(@TempDir & "\ECO_2.log", 0)
	If $file = -1 Then
...
While 1
    $arch = FileFindNextFile($search) 
    If @error Then ExitLoop;cuando no hay mas archivos sale del loop
;    MsgBox(4096, "$arch:", $arch)
	$t =  FileGetTime($arch, 0, 1)
	if $t>$FRef Then
	_ArrayAdd($Listafechas, $t);si es mayor que la fecha buscada se agrega a la lista de fechas (en el array)
EndIf
WEnd
;_ArrayDisplay($Listafechas, "Antes")
_ArraySort($Listafechas);se ordena la lista de fechas
...
;If $search = -1 Then
   ...
   ; Exit
;EndIf
$arch_log = $Ruta_IMAGOJO&'\Log\'&@YEAR&@MON&@MDAY&'_2.log';Creación de ruta y archivo log diario
$n=0
...
While 1;bloque de envío 
	...
	;Se van seleccionado los archivos en la carpeta.
		If $search = -1 Then; -1 indica que no hay archivos en la carpeta
		MsgBox(0, "IMAGOJODIR", "Fueron movidos los DICOM de la carpeta",5); recien movidos porque sabíamos que había archivos
		Exit
		EndIf
	$w=0;se inicia contador de archivos con la misma fecha
	While 1;se busca en todos los archivos de la carpeta
	$arch = FileFindNextFile($search) 
		If @error Then ExitLoop;Si no hay mas archivos sale del loop
	$t =  FileGetTime($arch, 0, 1), ;0 = Modificado, 1 = retorna una cadena YYYYMMDDHHMMSS
	if $t>$FRef Then
...
;				FileDelete ($Ruta_dcm&"\"&$arch);En esta función no se borran los archivos
				...
				Sleep (150)
				IniWrite($Ruta_IMAGOJO&"\IMAGOJO2.ini", "Fechas", "FRef", $Listafechas[$j])
