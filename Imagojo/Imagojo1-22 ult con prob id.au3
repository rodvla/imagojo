...
Global $Pav=0, $arch_log, $Ocultar, $estudioSel="", $Consulta=0, $Nro_registros, $Nro_CONSULTA, $Query, $Pat_id_issuer_Pacsdb=''
...
$Y_vent=$size[1]
	...
	ElseIf ProcessExists("GoogleChromePortable.exe") Then
		...
		SplashTextOn ("Abrir Visor Web", "Abriendo", 250,100)
...
$emi = GUICtrlCreateInput($Pat_id_issuer_Pacsdb, 498, 64, 57, 21)
...
If $Ced_pa='' And $Nom_pa='' Then $Consulta=0
...
$Label11 = GUICtrlCreateLabel("Emisor", 430, 64, 63, 28)
GUICtrlSetFont(-1, 15, 400, 0, "MS Sans Serif")
...
Case $OK1; Crear
	...
	$Pat_id_issuer_Pacsdb= GUICtrlRead($emi)
...
Case $Consultar;Consulta a la base de datos para ver si ya existe el paciente, y en caso afirmativo trae los datos
	...
	If $Ced_pa then 
		;RunWait( @comspec & " /c dcmqr -L ESTACION1@LOCALHOST:11113 "&$AE1&"@"&$IpPuertoServidor & " -q00100020="&$Ced_pa&"* -r 00100030 -r 00100010 -r 00100021 -r 00100040 > " & @TempDir & "\ImagojoQuery.dat",$dir_trab,@SW_HIDE);consulta por id paciente
		RunWait( @comspec & " /c dcmqr -L ESTACION1@LOCALHOST:11113 "&$AE1&"@"&$IpPuertoServidor & " -q00100020="&$Ced_pa&"* -r 00100030 -r 00100010 -r 00100040 > " & @TempDir & "\ImagojoQuery.dat",$dir_trab,@SW_HIDE);consulta por id paciente
...
;Send ("dcmqr -L QRSCU:11113 DCM4CHEE@localhost:11112 -q00100020=1512* -r 00100030 -r 00100010 -r 00100021 -r 00100040{ENTER}")
	...
	;MsgBox(0, "El nombre es :"&$array[0], $array[0] )
	IF $array[0]>1 THEN
	$Nom_pa = $array[2]
	Else
	$Nom_pa = ''
	...
	$result = StringInStr($Query, "(0010,0021)",0,$Nro_Consulta+1) ;busca el dato, se saltea la 1ra. porque es la propia consulta
	$var = StringMid($Query, $result, 80)
	$array = StringSplit($var, '[', 1)
	$var = $array[2];selecciono el dato desde este caracter al final
	$array = StringSplit($var, ']', 1)
...
FileWriteLine ($file, "00100021:"&$Pat_id_issuer_Pacsdb)
