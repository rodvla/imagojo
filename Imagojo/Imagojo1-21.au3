...
Global $Pav=0, $arch_log, $Ocultar, $estudioSel="", $Consulta=0, $Nro_registros
...
$Label20 = GUICtrlCreateLabel("1 de 3", 600, 354, 56, 28)
GUICtrlSetFont(-1, 15, 400, 0, "MS Sans Serif")
...
Case $Consultar;Consulta a la base de datos para ver si ya existe el paciente, y en caso afirmativo trae los datos
	...
	If $Ced_pa then 
		RunWait( @comspec & " /c dcmqr -L QRSCU:11113 "&$AE1&"@"&$IpPuertoServidor & " -q00100020="&$Ced_pa&"* -r 00100030 -r 00100010 -r 00100021 -r 00100040 > " & @TempDir & "\ImagojoQuery.dat",$dir_trab,@SW_HIDE);consulta por id paciente
	;Send ("dcmqr -L QRSCU:11113 DCM4CHEE@localhost:11112 -q00100020=1512* -r 00100030 -r 00100010 -r 00100021 -r 00100040{ENTER}")
	Else 
		RunWait( @comspec & " /c dcmqr -L QRSCU:11113 "&$AE1&"@"&$IpPuertoServidor & " -q00100010="&$Ape_pa&"* -r 00100030 -r 00100020 -r 00100021 -r 00100040 > " & @TempDir & "\ImagojoQuery.dat",$dir_trab,@SW_HIDE);consulta por apellido
	;RunWait( @comspec & " /c dcmecho "&$AE1&"@"&$IpPuertoServidor & " > " & @TempDir & "\ECO.log",$dir_trab,@SW_HIDE)
	EndIf
	...
	$result = StringInStr($chars, "INFO   - Received ") ;busca la posición donde está el dato
	$result = $result+18;se posiciona exactamente donde comienza el dato
	$var = StringMid($chars, $result, 6) ; extraigo 6 caracteres por si es un número largo
	...
	if 	$Nro_registros >1 Then MsgBox(0, "La cantidad de registros encontrados es:", $Nro_registros)
	FileClose($file)
	...
	$result = StringInStr($chars, "(0010,0010)",0,2) ;busca la 2º ocurrencia del dato, la 1ra. es la propia consulta
	$var = StringMid($chars, $result, 80)
	...
	$result = StringInStr($chars, "(0010,0020)",0,2) ;busca la 2º ocurrencia del dato, la 1ra. es la propia consulta
	$var = StringMid($chars, $result, 80)
	$array = StringSplit($var, '[', 1)
	$var = $array[2];selecciono el dato desde este caracter al final
	$array = StringSplit($var, ']', 1)
	...
	;(0010,0021) LO #0 [] Issuer of Patient ID
	$result = StringInStr($chars, "(0010,0021)",0,2) ;busca la 2º ocurrencia del dato, la 1ra. es la propia consulta
	$var = StringMid($chars, $result, 80)
	$array = StringSplit($var, '[', 1)
	$var = $array[2];selecciono el dato desde este caracter al final
	$array = StringSplit($var, ']', 1)
	$pat_id_issuer_Pacsdb = $array[1]
	;MsgBox(0, "El emisor del ID es :", $pat_id_issuer_Pacsdb)

	;(0010,0030) DA #0 [] Patient's Birth Date
	$result = StringInStr($chars, "(0010,0030)",0,2) ;busca la 2º ocurrencia del dato, la 1ra. es la propia consulta
	$var = StringMid($chars, $result, 80)
	$array = StringSplit($var, '[', 1)
	$var = $array[2];selecciono el dato desde este caracter al final
	$array = StringSplit($var, ']', 1)
	...
	$result = StringInStr($chars, "(0010,0040)",0,2) ;busca la 2º ocurrencia del dato, la 1ra. es la propia consulta
	$var = StringMid($chars, $result, 80)
	$array = StringSplit($var, '[', 1)
	$var = $array[2];selecciono el dato desde este caracter al final
	$array = StringSplit($var, ']', 1)
